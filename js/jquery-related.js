 ...
 * jQuery 插件 级联下拉列表
 * 作者：刘百泉
 * 时间：2010年12月16日 14:06:58
 **/
(function($){
	var arr1 =['安徽','澳门','北京','福建','甘肃','广东','广西','贵州','海南','河北','河南','黑龙江','湖北','湖南','吉林','江苏','江西','辽宁','内蒙古','宁夏','青海','山东','山西','陕西','上海','四川','台湾','天津','西藏','香港','新疆','云南','浙江','重庆','海外'];
/********** 市级数据 **********/
var arr2 = [
...
$.fn.related = function(options){
	options=$.extend({},$.fn.related.defaults,options);

	var _self = $(this),
		province = _self.find(options.S1),
		city = _self.find(options.S2),
		area = _self.find(options.S3);

	function initOptions(element, data, initValue) {
		var innerHTML = [];

		if(initValue == "请选择"){
			innerHTML.push('<option value="请选择">请选择</option>');
		}

		for(var i = 0, l = data.length; i < l; i++){
			innerHTML.push('<option value="',data[i],'" ',(data[i] == initValue ? "selected" : ""),'>',data[i],'</option>');
		}
		element.html(innerHTML.join(""));
	}

	initOptions(province, arr1, options.S1Text);
	var proIndex = arr1.getIndex(options.S1Text);

	if(proIndex == -1){
		initOptions(city, [], options.S2Text);
		initOptions(area, [], options.S3Text);
	} else {
		initOptions(city, arr2[proIndex], options.S2Text);
		var cityIndex = arr2[proIndex].getIndex(options.S2Text);
		if(cityIndex == -1){
			initOptions(area, []);
		} else {
			initOptions(area, arr3[proIndex][cityIndex], options.S3Text);
		}
	}
	...
	var prochange = function () {
			...
			if(value == "请选择"){
				initOptions(city, [], "请选择");
				initOptions(area, [], "请选择");
			} else {
				var proIndex = arr1.getIndex(value),
					cityValue = arr2[proIndex][0],
					cityIndex = arr2[proIndex].getIndex(cityValue);
				initOptions(city, arr2[proIndex]);
				initOptions(area, arr3[proIndex][cityIndex]);
			}
		},
		cityChange = function () {
			var value = $(this).val();
			...
			if(value == "请选择"){
				...
				var proValue = province.val(),
					proIndex = arr1.getIndex(proValue),
					cityIndex = arr2[proIndex].getIndex(value);
	...
	province.off('change', prochange).on("change", prochange);
	city.off('change', cityChange).on("change", cityChange);
}
$.fn.related.defaults={
	S1:"",
	S2:"",
